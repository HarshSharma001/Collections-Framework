...
import rootLevelInterfaces.MyCollection;
import rootLevelInterfaces.childLevelInterfaces.MySet;
...
public class MyConcreteSet implements MySet {
	 ...
	 */
	public MyConcreteSet(String type) {
		this.type = type;
	 ...
	 */
	public void printArray(Object[] argArray) {
		System.out.print("[");
	...
	/**
	 * method to copy old elements after increasing the size of the set
	 * 
	 * @param tempArr an array containing old array elements
	 * @param array   the new array with increased size
	 * @return Object[] type array
	 */
	private Object[] copyPreviousElements(Object[] tempArr, Object[] array) {
		for (int i = 0; i < tempArr.length && tempArr[i] != null; i++) {
			array[i] = tempArr[i];
		}
		return array;
	}
	 ...
	 */
	private Object[] copyOldArrayElementsWithNewElement(Object[] array, Object o) {
		if (currentSize >= 1) {
	 ...
	 */
	private void updateSortingStatus(int i, int j, String type, Object[] array) {
		if (type.equals("int") || type.equals("Integer")) {
	 ...
	 * @return Object[] array
	 */
	private Object[] saveArrayELements(Object[] array) {
	 ...
	 */
	private Object[] increaseSize(Object[] array) {
		if (type.equals("int") || type.equals("Integer")) {
	...
	/**
	 * public method to print the dynamic set from outside
	 */
	public void print() {
		System.out.print("[");
	 ...
	 * @throws Exception - The array datatype can't be determined
	 */
	public void sortWithStabilty() throws Exception {
		Object[] tempArr = new Object[currentSize];
		for (int i = 0; i < tempArr.length; i++) {
			tempArr[i] = array[i];
		}
	...
	/**
	 * method to findout if the set is sorted or not
	 * 
	...
	private void updateSortingStatusAfterReplacement(int left, int mid, int right, Object[] array, String type) {
		if (type.equals("int") || type.equals("Integer")) {
	...
	/**
	 * 
	 * @param a first integer element
	 * @param b second integer element
	 * @return integer element as minimum out of two numbers
	 */
	...
	public void add(Object o) {
		...
		if (currentSize < array.length) {
			...
			array = copyPreviousElements(tempArr, array);
			array = copyOldArrayElementsWithNewElement(array, o);
		}
	...
	public void remove(Object o) throws Exception {
		if (isSorted) {
			...
			if (location == -1) {
				System.out.println();
				throw new Exception("The specified object isn't present in the set");
			} else {
				int index = 0;
	...
	public Object[] retainAll(MyCollection mc) {
		...
		if (isSorted) {
			...
			Object[] temp = new Object[arr_index];
			...
			for (Object obj : tempArr) {
				if (linearSearch.search(obj, currentSize, array) != -1) {
					ansArr[index] = obj;
					index++;
				}
			}
	...
	@Override
	public Object[] findUnion(MySet set) {
		Object[] tempArr = set.toArray();
		Object ansArr[] = new Object[tempArr.length + currentSize];
		int i = 0;
		for (Object obj : tempArr) {
			ansArr[i] = obj;
			i++;
		...
		for (int j = 0; j < currentSize; j++) {
			if (linearSearch.search(array[j], i, ansArr) == -1) {
				ansArr[i] = array[j];
				i++;
			}
		}
	...
	@Override
	public Object[] findIntersection(MySet set) {
		Object[] tempArr = set.toArray();
		int size = getMin(tempArr.length, currentSize);

		Object[] ansArr = new Object[size];

		if (isSorted) {
			int index = 0;
			for (Object obj : tempArr) {
				try {
					if (binarySearch.search(obj, currentSize, array, type) != -1) {
						ansArr[index] = obj;
						index++;
					}
				} catch (Exception e) {
